...
interface ProxyProviderSubscription {
  id: number;
  name: string;
  providers: number[];
...
const ProxyProviderSubscriptionManager: React.FC = () => {
  ...
  const [editingSub, setEditingSub] = useState<ProxyProviderSubscription | null>(null);
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetch(`/api/subscriptions/proxy-providers/${id}`, { method: 'DELETE' });
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={3}>Proxy Provider Subscriptions</Title>
      ...
      <Table
        ...
        columns={[
          ...
          { title: 'Providers', key: 'providers', render: (_: any, r: ProxyProviderSubscription) => r.providers.map(id => providerMap[id] || id).join(' → ') },
          { title: 'Internal Group', key: 'group', render: (_: any, r: ProxyProviderSubscription) => groupMap[r.internal_proxy_group_id] || r.internal_proxy_group_id },
...
export default ProxyProviderSubscriptionManager;
